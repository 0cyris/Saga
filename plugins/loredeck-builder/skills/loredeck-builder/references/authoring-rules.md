...
The full contract is `$CLAUDE_PLUGIN_ROOT/docs/SAGA_LOREDECK_SCHEMA.md`; authoring intent is `$CLAUDE_PLUGIN_ROOT/docs/LOREDECK_AND_LORECARD_CREATION_GUIDE.md`. Read the schema doc before emitting deck JSON. These are the rules that matter most in practice, including ones the health engine enforces.
...
**Name entry files by topic, not by generation batch.** The schema doesn't care how many files exist or what they're called — `stats --write` will happily accept `entries/batch-1.json`, `entries/batch-2.json`, etc. But generic batch-numbered files are a maintenance dead end: nobody can tell what's inside one without opening it, diffs are noisy, and later edits have nowhere obvious to go. Use category subfolders instead — `characters/`, `events/`, `rules/`, `knowledge/`, `secrets/`, `relationships/`, `locations/`, `factions/` — mirroring the bundled reference decks (`$CLAUDE_PLUGIN_ROOT/reference-decks/hp-core`: `ages/`, `characters/`, `knowledge_gates/`, `places/`, `spell_gates/`, etc.). If a category grows large, split it further by sub-topic (`characters/adult_baselines.json`, `characters/core_students.json`), never by arbitrary sequence number.

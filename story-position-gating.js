/**
 * story-position-gating.js -- Saga/Wandlight
 * Pure helpers for evaluating entry-level Story Position gates.
 */

import { normalizeLoreEntry } from './lore-matrix.js';

export const POSITION_GATE_STATUSES = Object.freeze({
    NO_GATE: 'no_gate',
    MATCH: 'match',
    MISMATCH: 'mismatch',
...
function getEntryPackId(entry = {}) {
    return cleanString(
        entry.extensions?.sagaLorepack?.packId
        || entry.lorepackId
        || entry.packId
...
function getContextForEntry(entry = {}, state = {}, options = {}) {
    ...
    const contexts = isPlainObject(options.lorepackContexts)
        ? options.lorepackContexts
        : isPlainObject(state?.lorepackContexts)
            ? state.lorepackContexts
            : {};
    return {
        packId,
...
function getContextSortKey(index = null, context = {}, packId = '') {
    const direct = Number(context?.sortKey ?? context?.positionSortKey);
...
function anchorMatches(position = {}, context = {}, index = null, packId = '') {
    ...
    if (!contextAnchor) {
        return { ok: false, unresolved: true, reason: `Entry requires anchor ${requiredAnchor}, but this Lorepack has no selected anchor.` };
    }
...
function windowMatches(position = {}, context = {}, index = null, packId = '') {
    ...
    if (contextSort === null) {
        ...
        return { ok: false, unresolved: true, reason: 'Entry has a position window, but this Lorepack Story Position has no comparable anchor or sort key.' };
    }

    if (fromSort !== null && contextSort < fromSort) {
        return { ok: false, reason: `Current Story Position is before ${fromAnchorId || fromSort}.` };
    }
    if (toSort !== null && contextSort > toSort) {
        return { ok: false, reason: `Current Story Position is after ${toAnchorId || toSort}.` };
    }
    return { ok: true };
}
...
function textFieldMatches(entryValue, contextValue, label) {
    ...
    if (!contextText) return { ok: false, unresolved: true, reason: `Entry requires ${label} ${entryValue}, but this Lorepack Story Position has no ${label}.` };
...
function stardateMatches(position = {}, context = {}) {
    ...
    if (!Number.isFinite(current)) {
        return { ok: false, unresolved: true, reason: 'Entry has a stardate gate, but the Lorepack Story Position has no stardate.' };
    }
...
function coordinateMatches(coordinate = {}, context = {}) {
    ...
    if (id && !contextText) {
        return coordinate.required === false
            ? { ok: true }
            : { ok: false, unresolved: true, reason: `Entry requires coordinate ${axis}:${coordinate.id}, but Story Position has no matching coordinate.` };
    }
...
export function evaluateEntryPositionGate(entryInput = {}, state = {}, options = {}) {
    const entry = normalizeLoreEntry(entryInput);
    const position = entry.position || {};
    const coordinates = Array.isArray(entry.coordinates) ? entry.coordinates : [];

    if (!hasAnyPositionGate(position, coordinates)) {
        return {
            status: POSITION_GATE_STATUSES.NO_GATE,
            eligible: true,
            hasGate: false,
            reason: 'Entry has no Story Position gate.',
    ...
    if (!context) {
        return {
            status: POSITION_GATE_STATUSES.UNRESOLVED,
            eligible: options.unresolvedEligible !== false,
            hasGate: true,
            reason: packId
                ? `Entry has Story Position gates, but ${packId} has no Story Position context.`
                : 'Entry has Story Position gates, but no pack id was available for context lookup.',
            packId,
            entry,
        };
    }
    ...
    if (failed) {
        return {
            status: failed.unresolved ? POSITION_GATE_STATUSES.UNRESOLVED : POSITION_GATE_STATUSES.MISMATCH,
            eligible: failed.unresolved ? options.unresolvedEligible !== false : false,
            hasGate: true,
            reason: failed.reason || 'Entry Story Position gate does not match the active Lorepack context.',
            packId,
            context,
            entry,
        };
    ...
    return {
        status: POSITION_GATE_STATUSES.MATCH,
        eligible: true,
        hasGate: true,
        reason: 'Entry Story Position gate matches the active Lorepack context.',
        packId,
        context,
        entry,
    };
...
export function evaluateEntryPositionGates(entries = [], state = {}, options = {}) {
    return (Array.isArray(entries) ? entries : []).map(entry => evaluateEntryPositionGate(entry, state, options));
}

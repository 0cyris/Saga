...
import { getState, getLoredeckLibraryRegistry } from '../state/state-manager.js';
import { fetchJson, loadLoredeckStackSources, mergeLoredeckTimelineRegistries } from '../loredecks/loredeck-loader.js';
...
function normalizeTimelineAnchor(raw = {}, source = {}, index = 0) {
    ...
    return {
        ...
        stardate: cleanString(raw.stardate, 80),
        stardateFrom: cleanString(raw.stardateFrom || raw.stardateStart, 80),
        stardateTo: cleanString(raw.stardateTo || raw.stardateEnd, 80),
...
function getAnchorSearchText(anchor = {}) {
    return [
        ...
        anchor.stardateFrom,
        anchor.stardateTo,
        ...(anchor.coordinates || []).flatMap(coordinate => [coordinate.axis, coordinate.value]),
        ...(anchor.aliases || []),
        ...(anchor.tags || []),
    ].filter(Boolean).join(' ');
}

...
const LOREDECK_PACKAGE_ROOT = 'loredecks';
const LOREDECK_INDEX_PATH = `${LOREDECK_PACKAGE_ROOT}/index.json`;
...
function joinZipPath(base = '', child = '') {
  ...
  if (!raw) return '';
  if (raw.startsWith(`${LOREDECK_PACKAGE_ROOT}/`)) return assertSafeZipEntryPath(raw);
...
function normalizeIndexRecords(index = {}) {
  const records = asArray(index.loredecks);
  return records
...
function getManifestRefForRecord(record = {}) {
  ...
  if (manifest) return manifest.startsWith(`${LOREDECK_PACKAGE_ROOT}/`) ? manifest : `${LOREDECK_PACKAGE_ROOT}/${manifest}`;
  return packId ? `${LOREDECK_PACKAGE_ROOT}/${packId}/loredeck.json` : '';
}
...
export async function parseLoredeckZipPackage(input, options = {}) {
  ...
  const index = await readOptionalJson(archive, LOREDECK_INDEX_PATH);
  const warnings = [];

...
assert(panel.includes('createContextCommandCenterCard'), 'Context tab must render the Phase 6 command center.');
assert(panel.includes('Runtime Context'), 'Context command center must use runtime Context language.');
assert(panel.includes('Browse Context'), 'Context command center must expose the Context Browser as a primary action.');
assert(panel.includes('Review Proposals'), 'Context command center must expose proposal review as a primary action.');
assert(panel.includes("['assisted', 'Assisted'"), 'Context automation must expose Assisted mode.');
assert(panel.includes('contextDetectionAutoMinTurns'), 'Context automation must expose minimum-turn cadence settings.');
assert(panel.includes('contextDetectionAutoCharacterThreshold'), 'Context automation must expose character-threshold cadence settings.');
assert(panel.includes('contextReasonerFallbackEnabled'), 'Context automation must expose a Reasoner fallback toggle.');
assert(panel.includes('contextLocalApplyMinConfidence'), 'Context automation must expose local auto-apply confidence.');
assert(panel.includes('contextReasonerProposalMinConfidence'), 'Context automation must expose bounded Reasoner proposal confidence.');
assert(panel.includes('CONTEXT_PROPOSAL_REVIEW_ID'), 'Context proposal review must use a stable fullscreen overlay id.');
assert(panel.includes('openContextProposalReview'), 'Context command center must open the proposal review overlay.');
assert(panel.includes('createContextProposalReviewShell'), 'Context proposal review must render a dedicated fullscreen shell.');
assert(panel.includes('Apply All'), 'Context proposal review must expose an apply-all action.');
assert(panel.includes('Dismiss All'), 'Context proposal review must expose a dismiss-all action.');
assert(panel.includes('createContextProposalReviewRow'), 'Context proposal review must render per-proposal rows.');
assert(panel.includes('formatContextPatchSummary'), 'Context proposal review must summarize proposed Context patches.');
assert(panel.includes('createContextAdvancedBriefSection'), 'Context tab must move legacy global fields into an advanced brief section.');
assert(panel.includes("'context.advancedBrief'"), 'Advanced Context Brief section must have a stable collapsible id.');
assert(!panel.includes('container.appendChild(createContextDetectionCard(state));'), 'Context tab must not render the old standalone detection card.');
assert(panel.includes('toggleLoredeckContextManualLock'), 'Loaded Loredeck Context rows must expose a direct lock/unlock control.');
assert(panel.includes('Seed From Brief'), 'Loaded Loredeck Context rows must seed from the rich Context Brief projection.');
assert(panel.includes('getContextBriefSignalSummary'), 'Advanced Context Brief must summarize non-date detector signals.');
assert(panel.includes('createContextBriefStatusCard'), 'Context tab must surface the latest Context Brief detector status.');
assert(panel.includes("markTourTarget(row, 'context.briefStatus')"), 'Context Brief status row must have a tour target.');
assert(panel.includes('state?.contextBrief'), 'Context Brief status UI must read from chat state.');
assert(panel.includes('refreshPanelBody({ preserveScroll: true, preserveWindowScroll: true });'), 'Context detection completion must preserve runtime scroll position.');
assert(style.includes('wandlight-context-brief-status'), 'Context Brief status row must have dedicated compact styling.');
assert(panel.includes('createContextResolutionAuditPanel'), 'Context tab must surface the latest resolver audit summary.');
assert(panel.includes('contextResolutionAudit'), 'Context resolver audit metadata must be persisted in runtime panel state.');
assert(panel.includes('createContextAutomationAuditPanel'), 'Context tab must surface the latest background automation audit summary.');
assert(panel.includes('contextAutomationAudit'), 'Context automation audit metadata must be persisted in runtime panel state.');
assert(panel.includes('Last Automation Check'), 'Context automation audit must use clear user-facing copy.');
assert(panel.includes('Reasoner choices remain review proposals through alpha'), 'Context automation UI must state the alpha Reasoner review-only policy.');
assert(panel.includes('storeContextResolutionProposalsFromResult'), 'Manual Context Reasoner results must persist reviewable proposals.');
assert(panel.includes("if (result?.status === 'in_flight') return getContextResolutionProposals().length;"), 'Manual duplicate in-flight Context checks must preserve existing proposals.');
assert(panel.includes('contextResolutionProposalMeta'), 'Manual Context proposal metadata must be stored for the review panel.');
assert(panel.includes("'manual_reasoner_cached'"), 'Manual Context proposal metadata must identify cached Reasoner results.');
assert(panel.includes('Apply Proposals'), 'Context tab must expose reviewable proposal application.');
assert(panel.includes('clearContextResolutionProposals();'), 'Context proposal application/dismissal must clear review state explicitly.');

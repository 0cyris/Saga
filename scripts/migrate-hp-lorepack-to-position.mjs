...
function cleanLegacyTerminology(value = '') {
  return String(value || '')
    .replace(/date- and character-gated/g, 'Story Position- and character-gated')
    .replace(/date-gated references/g, 'Story Position-gated references')
    .replace(/date-gated/g, 'Story Position-gated')
    .replace(/date constraint/g, 'Story Position constraint')
    .replace(/date-aware/g, 'Story Position-aware');
}
...
function inferPositionScope(spanDays = 0) {
...
function buildPositionLabel(range = {}, from, to, spanDays = 0) {
...
export function buildPositionFromDateRange(range = {}, options = {}) {
  ...
  return sortObjectKeysForPosition({
    scope: inferPositionScope(spanDays),
    ...
    label: buildPositionLabel(range, orderedFrom, orderedTo, spanDays),
...
function buildRetrieval(entry = {}, position = {}) {
  ...
  return omitEmptyObject({
    ...
    positionalBoost: wide ? 'low' : narrow ? 'high' : 'medium',
    triggers,
  }) || {};
}

function normalizeVisibilityMapToPosition(value = {}) {
  ...
  for (const [name, rawPosition] of Object.entries(map)) {
    ...
    const point = parsePositionDate(text);
...
function mergeVisibility(entry = {}) {
  ...
  const known = normalizeVisibilityMapToPosition(raw.knownBy ?? entry.whoKnowsTruth);
  const notKnown = normalizeVisibilityMapToPosition(raw.notKnownByBefore);
  const existingKnownByAtPosition = isPlainObject(raw.knownByAtPosition) ? raw.knownByAtPosition : {};
  const existingNotKnownByBeforePosition = isPlainObject(raw.notKnownByBeforePosition) ? raw.notKnownByBeforePosition : {};
  ...
  const out = {
    ...
    knownByAtPosition: { ...existingKnownByAtPosition, ...known.positions },
    notKnownByBeforePosition: { ...existingNotKnownByBeforePosition, ...notKnown.positions },
    ...
    publicFromPosition: raw.publicFromPosition || parsePositionDate(raw.publicFrom),
    secretUntilPosition: raw.secretUntilPosition || parsePositionDate(raw.secretUntil),
  };
  return omitEmptyObject(out) || {};
}
...
function copyKnownFields(entry = {}, position = {}) {
  const out = {};
  ...
  assign('retrieval', buildRetrieval(entry, position));
...
export function migrateEntryToPosition(entry = {}, options = {}) {
  if (isPlainObject(entry.position) && Object.keys(entry.position).length) {
    return {
      entry: copyKnownFields(entry, entry.position),
      changed: true,
      status: 'position_native_existing',
    };
  }
  const range = getEntryDateRange(entry);
  const position = buildPositionFromDateRange(range, { timeline: options.timeline });
  if (!position) {
...
function updateManifest(root, entryFiles, options = {}) {
  const manifestFile = path.join(root, 'lorepack.json');
  ...
  const next = {
    ...
    description: String(manifest.description || '').replace('date', 'Story Position'),
...
export function migrateLorepackRoot(root = DEFAULT_ROOT, options = {}) {
  ...
  for (const file of files) {
    ...
    const nextEntries = entries.map(entry => {
      report.entryCount += 1;
      const result = migrateEntryToPosition(entry, { ...options, timeline });
...
function main() {
  ...
  console.log(JSON.stringify({
    ...
    note: options.write
      ? 'HP Lorepack entries were rewritten to Story Position-native schema v3. Legacy entry date gates were removed.'
      : 'Dry run only. Pass --write to rewrite HP Lorepack entries to Story Position-native schema v3.',
  }, null, 2));
}

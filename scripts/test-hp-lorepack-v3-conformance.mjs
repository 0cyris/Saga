...
const ROOT = 'Lorepacks/hp-golden-trio';
...
const manifest = readJson(path.join(ROOT, 'lorepack.json'));
...
for (const record of entryFiles) {
  ...
  for (const entry of record.json.entries) {
    ...
    assert.ok(entry.position && typeof entry.position === 'object', `${entry.id} must have position`);
    assert.ok(['anchor', 'window', 'global'].includes(entry.position.scope), `${entry.id} must declare position.scope`);
    assert.equal(Number.isFinite(Number(entry.position.sortKeyFrom)), true, `${entry.id} must have position.sortKeyFrom`);
    assert.equal(Number.isFinite(Number(entry.position.sortKeyTo)), true, `${entry.id} must have position.sortKeyTo`);
    assert.ok(entry.position.precision, `${entry.id} must have position.precision`);
    assert.ok(entry.position.label, `${entry.id} must have position.label`);
    ...
    assert.ok(entry.retrieval?.positionalBoost, `${entry.id} must have retrieval.positionalBoost`);
    if (entry.position.scope === 'global' || ['series', 'wide'].includes(entry.position.windowKind)) {
      widePositionCount += 1;
      ...
      assert.equal(entry.retrieval.positionalBoost, 'low', `${entry.id} wide lore should have low positional boost`);
    }
  }
}
...
console.log('HP Lorepack v3 conformance tests passed.');

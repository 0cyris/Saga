...
import { DEFAULT_HP_LOREDECK_IDS } from '../loredeck-defaults.js';
...
async function repairEntryFiles(deckId, manifest) {
  ...
  for (const rel of manifest.files || []) {
    const file = path.join(root, 'Loredecks', deckId, rel);
...
async function writeTagRegistry(deckId, usedTags) {
  const tagsPath = path.join(root, 'Loredecks', deckId, 'tags.json');
  if (!usedTags.length) {
    try {
...
async function repairManifestFiles(deckId, hasTags) {
  for (const name of ['loredeck.json', 'manifest.json']) {
    const file = path.join(root, 'Loredecks', deckId, name);
...
async function repairEpilogueTimeline() {
  const file = path.join(root, 'Loredecks', 'hp-epilogue-post-war', 'timeline.json');
...
for (const deckId of DEFAULT_HP_LOREDECK_IDS) {
  const manifest = await readJson(path.join(root, 'Loredecks', deckId, 'loredeck.json'));

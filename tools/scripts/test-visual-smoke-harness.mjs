...
assert(liveSmoke.includes('measureMobileLorecardEditorLatency') && liveSmoke.includes('postHoldReadyMs') && liveSmoke.includes('duplicateContextmenuReopened') && liveSmoke.includes('closeGoneMs') && liveSmoke.includes('footerButtonsVerticallyAligned') && liveSmoke.includes('editorFooterState'), 'Mobile Lorecard editor smoke must measure long-press open, Save/Close footer alignment, and Close latency in the browser.');
...
assert(
    lorecardsPanel.includes("if (mobileShell && !entry.isPending) card.classList.add('saga-lore-entry-card-tappable');") &&
        ...
        lorecardsPanel.includes('openAcceptedLorecardMobileEditor(entryId);') &&

...
assert(visualSmokeDoc.includes('Creator review-queue anchors/actions') && visualSmokeDoc.includes('Creator review-queue routing') && visualSmokeDoc.includes('Context proposal review overlay/actions'), 'Visual smoke runbook must document rendered follow-up for Creator review queue and Context proposal review coverage.');
...
assert(!runtimeShellView.includes("activeSubview?.id === 'session-story-opener'"), 'Story Opener must not special-case mobile shell Back behavior because it renders as a closeable Session dropdown.');
...
assert(lorecardsPanel.includes('const LORE_SOURCE_FILTER_OPTIONS = Object.freeze') && lorecardsPanel.includes("['creator', 'Creator Drafts']") && lorecardsPanel.includes("['context-suggestion', 'Context Suggestions']") && lorecardsPanel.includes('function getLoreSourceOriginMeta(entry = {})') && lorecardsPanel.includes("label: 'Source: Creator'") && lorecardsPanel.includes("label: 'Source: Context Suggestion'") && lorecardsPanel.includes('Creator Lorecard draft waiting for review before it becomes an Accepted Lorecard.') && lorecardsPanel.includes('Context-aware suggestion waiting for review before it becomes an Accepted Lorecard.') && lorecardsPanel.includes('Unified Lorecards workspace. Pending, High, Elevated, and Muted are states in one list.') && lorecardsPanel.includes('before it becomes an Accepted Lorecard.') && !lorecardsPanel.includes('accepted memory') && !lorecardsPanel.includes('Accepted memory'), 'Pending Review source badges and filters must distinguish Creator drafts and Context suggestions in the shared review flow with Accepted Lorecards language.');
...
const desktopStoryOpenerPosition = runtimeAdvancedPanel.indexOf(
    'container.appendChild(createStoryOpenerSessionSection(state));',
    runtimeAdvancedPanel.indexOf('if (!isRuntimeMobileShell())')
);
...
assert(desktopStoryOpenerPosition > runtimeAdvancedPanel.indexOf('Automation Mode') && desktopStoryOpenerPosition < runtimeAdvancedPanel.indexOf('Session Metrics'), 'Story Opener Creator must sit after Session controls and before Session Metrics.');
assert(runtimeAdvancedPanel.includes('function createStoryOpenerSessionSection') && runtimeAdvancedPanel.includes("const sectionId = options.sectionId || 'session.storyOpener'") && runtimeAdvancedPanel.includes('createCollapsibleSection(\n        sectionId'), 'Story Opener Creator must render through a shared closeable dropdown section.');
assert(runtimeAdvancedPanel.includes("createStoryOpenerSessionSection(state, { sectionId: 'session.storyOpener.mobile' })") && !runtimeAdvancedPanel.includes('openSessionMobileStoryOpener'), 'Mobile Session must expose Story Opener as a closeable dropdown instead of a one-way subview button.');
assert(runtimeAdvancedPanel.includes("mobileSubview?.id === 'session-story-opener'") && runtimeAdvancedPanel.includes("createStoryOpenerSessionSection(state, { sectionId: 'session.storyOpener.mobile', defaultOpen: true })"), 'Legacy mobile Story Opener subview state must render the same dropdown section instead of dumping the raw panel.');
...
assert(runtimeStoryOpenerPanel.includes('Opening Shape Preset') && runtimeStoryOpenerPanel.includes('createSelectInput') && runtimeStoryOpenerPanel.includes('saga-story-opener-shape-select-field'), 'Story Opener Creator must expose a mobile-friendly Opening Shape preset select.');
...
assert(style.includes('.saga-story-opener-section') && style.includes('.saga-collapsible-card:has(.saga-story-opener-panel)') && style.includes('.saga-story-opener-stage-list .saga-loredeck-creator-stage-label') && style.includes('overflow-wrap: anywhere'), 'Story Opener Creator must wrap headers, workflow buttons, and stage labels inside the narrow Saga drawer.');
assert(style.includes('.saga-story-opener-session-picker') && style.includes('.saga-story-opener-session-row-active') && style.includes('.saga-story-opener-session-row > .saga-story-opener-delete-button') && style.includes('grid-template-columns: minmax(0, 1fr) max-content'), 'Story Opener saved-opener picker must stay compact and mark the active session.');
assert(style.includes('.saga-story-opener-stage-list') && style.includes('grid-auto-flow: column') && style.includes('grid-auto-columns: minmax(158px, 176px)') && style.includes('overflow-x: auto'), 'Story Opener workflow buttons must use an internal horizontal rail instead of clipping at the drawer edge.');
assert(style.includes('.saga-runtime-drawer .saga-story-opener-input-grid') && style.includes('.saga-runtime-mobile .saga-story-opener-input-grid') && style.includes('grid-template-columns: minmax(0, 1fr)') && style.includes('.saga-runtime-mobile .saga-story-opener-quick-row') && style.includes('display: none'), 'Story Opener drawer and mobile inputs must be one column and mobile must hide the quick-shape button cluster.');
assert(style.includes('.saga-runtime-mobile .saga-story-opener-panel .saga-story-opener-stage-list') && style.includes('grid-auto-flow: column') && style.includes('scroll-snap-type: x proximity'), 'Story Opener workflow buttons must scroll horizontally on mobile like Loredeck Creator.');
...
assert(loredecksTabPanel.includes("'Loredeck Library'") && loredecksTabPanel.includes("'In-Progress Creator Projects'") && !loredecksTabPanel.includes("'loredecks.libraryLaunch',"), 'Loredecks tab must keep the shared static Library launch card and Advanced Creator Projects section without a Library dropdown.');
...
assert(/\.saga-loredeck-creator-project-search,[\s\S]*?\.saga-loredeck-creator-project-move-select\s*\{[\s\S]*?font-size:\s*12px !important;[\s\S]*?line-height:\s*1\.25 !important;/.test(style) && /\.saga-runtime-mobile \.saga-loredeck-creator-project-shelf\s*\{[\s\S]*?font-size:\s*0\.9em;/.test(style) && /\.saga-runtime-mobile \.saga-loredeck-creator-project-title\s*\{[\s\S]*?font-size:\s*0\.82em;/.test(style), 'In-Progress Creator Projects controls and cards must pin compact local typography instead of inheriting oversized host form fonts.');

...
assert(lorecardsPanel.includes("if (isRuntimeMobileShell() && stats.allPendingCount) return 'pending';") && lorecardsPanel.includes("if (stage === 'pending') return isRuntimeMobileShell() ? Number(stats.allPendingCount || 0) : Number(stats.pendingCount || 0);") && lorecardsPanel.includes("if (!mobileShell || lifecycleStage === 'pending')"), 'Mobile Lorecards Pending page must own the full review queue while Generation stays creation-focused.');
...
assert(runtimeAdvancedPanel.includes('function createBasicSessionNextActionButton(readiness)') && runtimeAdvancedPanel.includes("createButton(`Next: ${row.actionLabel}`") && runtimeAdvancedPanel.includes('getBasicReadinessAction(row)') && runtimeAdvancedPanel.includes('basic && mobileRoot') && runtimeAdvancedPanel.includes("enabled && !nextAction ? 'saga-primary-button' : ''"), 'Basic mobile Session summary must expose the concrete next Start Checklist action on the root surface.');

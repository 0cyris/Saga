...
assert(visualSmokeDoc.includes('Accepted detail access through tap-hold or a visible affordance'), 'Visual smoke runbook must document rendered follow-up for Accepted tap-hold and visible detail access.');
assert(visualSmokeDoc.includes('Accepted') && visualSmokeDoc.includes('Accepted/Active object cards') && visualSmokeDoc.includes('Accepted tap/tap-hold/detail affordances'), 'Visual smoke runbook must document rendered follow-up for Accepted object-card management.');
...
assert(visualSmokeDoc.includes('Rendered Accepted object-action review') && visualSmokeDoc.includes('object-state chips') && visualSmokeDoc.includes('tap/tap-hold/detail affordances') && visualSmokeDoc.includes('without showing equal permanent row buttons'), 'Visual smoke runbook must document rendered follow-up for Accepted object-state interactions.');
...
assert(lorecardsPanel.includes("if (lifecycleStage === 'automation')") && lorecardsPanel.includes('createMobileLoreAutomationPage(state)') && lorecardsPanel.includes("settings.loreAutomationPaused === true ? 'Resume' : 'Pause'") && lorecardsPanel.includes('createLoreAutomationActivityList(state)') && lorecardsPanel.includes('createLoreAutomationStateBadge(entry)'), 'Mobile Lorecards Automation tab must render the full Lore Automation cockpit and per-card ownership status.');
...
assert(style.includes('.saga-lore-automation-header') && /\.saga-lore-automation-header \.saga-runtime-button\s*\{[\s\S]*?min-height:\s*30px;[\s\S]*?font-size:\s*0\.78em;/.test(style), 'Lore Automation reset defaults must be compact and right-aligned in the title row.');
assert(defaultSettings.includes('mobileLorecardListTagsVisible: false') && settingsStore.includes('merged.mobileLorecardListTagsVisible = merged.mobileLorecardListTagsVisible === true;') && runtimeSettingsTab.includes('Quality of Life') && runtimeSettingsTab.includes('mobileLorecardListTagsVisible'), 'Settings must expose a default-off mobile Quality of Life toggle for showing Lorecard list tags.');
...
assert(lorecardsPanel.includes('function createMobileLorecardEditorTagsControl') && lorecardsPanel.includes('saga-mobile-lorecard-tags-chip-stack') && lorecardsPanel.includes('getTags: () => tags.slice()') && style.includes('.saga-mobile-lorecard-tags-chip-stack') && style.includes('.saga-mobile-lorecard-tags-add-row'), 'Mobile Lorecard editor must edit tags as a single stacked chip list with remove chips and an add row.');
...
assert(!loredecksTabPanel.includes('Next:') && loredecksTabPanel.includes('getLoredecksOperatorActionLabel') && loredecksTabPanel.includes('return label;') && loredecksTabPanel.includes("'Open Loredeck Library'") && loredecksTabPanel.includes("'loredecks.operator.library'"), 'Mobile Loredecks operator summary must keep direct Library actions without checklist-style Next buttons.');
...
assert(loredecksTabPanel.includes('getLoredecksMobileSubview') && loredecksTabPanel.includes("pushRuntimeMobileSubview('loredecks'") && loredecksTabPanel.includes("title: 'Stack Details'") && loredecksTabPanel.includes("header.setAttribute('aria-label', 'Open Stack Details')") && loredecksTabPanel.includes('openLoredecksMobileDetails();') && !loredecksTabPanel.includes("createButton(\n                getLoredecksOperatorActionLabel('details'") && !loredecksTabPanel.includes("'Stack Details',\n                'Open active stack readiness"), 'Mobile Loredecks must keep Stack Details in a tappable summary object instead of standalone root buttons.');
...
assert(!loredecksTabPanel.includes('primaryMobileActionId') && loredecksTabPanel.includes("primaryActions.appendChild(markTourTarget(createButton(\n            'Open Loredeck Library'"), 'Mobile Loredecks root must not duplicate Open Loredeck Library when the dominant action already opens it.');
...
assert(
    lorecardsPanel.includes('isActive: isActiveLorecardEntry(entry)') &&
        lorecardsPanel.includes("if (!entry.isPending && activeNow) metaRow.appendChild(createBadge('active'") &&
    ...
    'Active, pinned, and muted Lorecards must be visible row states with detailed controls in the unified workspace.'
);
...
assert(
    lorecardsPanel.includes("if (mobileShell && !entry.isPending) card.classList.add('saga-lore-entry-card-tappable');") &&
        ...
        lorecardsPanel.includes('if (!workspaceRow && (!mobileShell || isExpanded)) {') &&
    ...
    'Mobile Accepted Lorecards must use visible active state and tap/tap-hold details without permanent card detail buttons.'
);

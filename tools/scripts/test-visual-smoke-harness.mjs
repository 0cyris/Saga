...
assert(
    visualSmokeDoc.includes("SAGA_SMOKE_TARGET='mobile-redesign-harness'")
        ...
        && visualSmokeDoc.includes('animated secondary `Generate | Automate | Lore` sub-tab bar')
...
assert(visualSmokeDoc.includes('Lorecards secondary `Generate | Automate | Lore` sub-tab bar') && visualSmokeDoc.includes('Lorecards secondary sub-tab animation and placement'), 'Visual smoke runbook must document rendered follow-up for Lorecards secondary sub-tab navigation.');
...
assert(/\| `360x740` \| Basic \|[^\n]*Lorecards secondary `Generate \| Automate \| Lore` sub-tab bar/.test(visualSmokeDoc), 'Visual smoke runbook must document the Lorecards secondary sub-tab bar in the 360px Basic matrix.');
...
assert(visualSmokeDoc.includes('bottom-owned Deck Maker actions') && visualSmokeDoc.includes('Health/Deck Maker bottom actions'), 'Visual smoke runbook must document rendered follow-up for Deck Maker bottom-owned actions.');
...
assert(visualSmokeDoc.includes('Health/Deck Maker close controls') && visualSmokeDoc.includes('reachable Health/Deck Maker close affordances'), 'Visual smoke runbook must document rendered follow-up for Health and Deck Maker close controls.');
...
assert(runtimeShellView.includes('function renderDesktopLorecardsFlyout') && runtimeShellView.includes("nav.className = 'saga-desktop-lorecards-flyout'") && runtimeShellView.includes('function updateDesktopLorecardsFlyoutPosition') && runtimeShellView.includes('schedule(() => updateDesktopLorecardsFlyoutPosition(root, nav));') && runtimeShellView.includes("tab.setAttribute('aria-haspopup', 'true')") && runtimeShellView.includes('selectRuntimeLorecardsStage(stage)') && style.includes('.saga-desktop-lorecards-flyout::before') && style.includes('top: calc(var(--saga-lorecards-flyout-center-y, 180px) - 63px)') && style.includes('animation: saga-desktop-lorecards-flyout-in'), 'Desktop Lorecards must expose a button-centered Generate/Automate/Lore flyout that visually branches from the rail tab.');
...
assert(runtimeShell.includes("export const MOBILE_LORECARDS_STAGES = Object.freeze(['generate', 'automation', 'lore']);") && runtimeShell.includes('export const RUNTIME_LORECARDS_STAGES = MOBILE_LORECARDS_STAGES;') && runtimeShell.includes('export function normalizeRuntimeMobileLorecardsStage') && runtimeShell.includes('export const normalizeRuntimeLorecardsStage = normalizeRuntimeMobileLorecardsStage;') && runtimeShell.includes("if (stage === 'pending' || stage === 'accepted' || stage === 'approved' || stage === 'active' || stage === 'cards') return 'lore';") && runtimeShell.includes("if (stage === 'suggested' || stage === 'generation') return 'generate';") && runtimeShell.includes('lorecardsStage: normalizeRuntimeLorecardsStage') && runtimeShell.includes('export function getRuntimeLorecardsStage') && runtimeShell.includes('export function selectRuntimeLorecardsStage(stage)') && runtimeShell.includes('export function selectRuntimeMobileLorecardsStage(stage)'), 'Runtime shell must own the shared Lorecards Generate, Automate, and Lore sub-tab state for desktop and mobile.');
assert(runtimeShell.includes('function normalizeMobileLoreSubviewStack(stack)') && runtimeShell.includes('.filter(item => !normalizeRuntimeMobileLorecardsStage(item?.params?.stage))'), 'Runtime shell must drop stale Lorecards stage subviews instead of reopening the old mobile pipeline flow.');
...
assert(/\.saga-mobile-lorecards-subtabs\s*\{[\s\S]*?grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/.test(style) && /\.saga-mobile-lorecards-subtabs-intro \.saga-mobile-lorecards-subtabs\s*\{[\s\S]*?animation:\s*saga-mobile-lorecards-subtabs-in/.test(style) && runtimeShellView.includes('mobileLorecardsSubtabsIntroSeen') && style.includes('.saga-runtime-mobile.saga-mobile-lorecards-subtabs-active') && style.includes('--saga-mobile-lorecards-subtab-height'), 'Mobile Lorecards sub-tabs must be a three-tab strip whose intro animation runs only on first Lorecards entry.');
...
assert(runtimeStoryOpenerPanel.includes('Copy Opener') && runtimeStoryOpenerPanel.includes('navigator.clipboard.writeText'), 'Story Maker must expose copy-to-clipboard output.');
...
assert(liveSmoke.includes('1 / 14') && liveSmoke.includes('3 / 14') && liveSmoke.includes('1 / 15') && liveSmoke.includes('11 / 19') && liveSmoke.includes('1 / 55') && liveSmoke.includes('1 / 165'), 'Guide harness smoke must verify Basic prepared, Deck Maker fallback, Basic full, Advanced module, and Advanced full walkthrough progress counts.');

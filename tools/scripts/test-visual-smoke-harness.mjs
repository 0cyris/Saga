...
assert(lorecardsPanel.includes("footer.className = 'saga-mobile-lorecard-editor-footer'") && lorecardsPanel.includes('actionsContainer: footer') && style.includes('.saga-mobile-lorecard-editor-footer'), 'Mobile Lorecard editor persistent actions must live in a bottom footer.');
...
assert(style.includes('container-type: inline-size') && /@container \(min-width:\s*920px\)[\s\S]*?\.saga-lorecard-workspace-body\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\)\s+minmax\(300px,\s*0\.56fr\)/.test(style) && style.includes('.saga-lorecard-detail-card .saga-lore-editor-textarea') && style.includes('.saga-lorecard-active-toggle-button') && style.includes('.saga-lorecard-row-active-toggle') && lorecardsPanel.includes('function createLorecardActiveToggleButton'), 'Desktop Lorecards workspace must use container-aware layout, readable detail editor sizing, and round glowing active toggles on rows/detail.');
assert(
...
assert(!lorecardsPanel.includes("if (!stats.acceptedCount) return 'suggested';") && lorecardsPanel.includes('function getLorecardWorkspaceRows') && lorecardsPanel.includes('function createLorecardWorkspaceDetailPane') && lorecardsPanel.includes('No Lorecards match the current filters.') && style.includes('.saga-lorecard-workspace-row.saga-lore-entry-active'), 'Blank and active Lorecards must show object state through the unified workspace instead of redundant lifecycle sections.');
...
assert(lorecardsPanel.includes('card.addEventListener(\'pointermove\'') && lorecardsPanel.includes('deltaX > 8 || deltaY > 8') && /saga-runtime-tab-body[\s\S]*?-webkit-overflow-scrolling:\s*touch !important;[\s\S]*?touch-action:\s*pan-y !important;/.test(style), 'Mobile Lorecard tap-hold handlers must cancel on finger movement so page scroll remains available.');
...
assert(lorecardsPanel.includes('function getActiveLorecardReason(entry = {})') && lorecardsPanel.includes("rows.push(['Why active', getActiveLorecardReason(entry)])") && lorecardsPanel.includes("entry.extensions?.autoRelevance?.reason"), 'Active Lorecards must show a concise Why active reason in the workspace detail view.');
...
assert(
    lorecardsPanel.includes('isActive: isActiveLorecardEntry(entry)') &&
        ...
        lorecardsPanel.includes("if (entry.isPinned) metaRow.appendChild(createBadge('pinned'") &&
        lorecardsPanel.includes("if (entry.isSuppressed) metaRow.appendChild(createBadge('muted'") &&
        lorecardsPanel.includes('function createLorecardActiveToggleButton') &&
        lorecardsPanel.includes("button.className = `saga-lorecard-active-toggle-button saga-lorecard-active-toggle") &&
        lorecardsPanel.includes("className: 'saga-lorecard-row-active-toggle'") &&
        lorecardsPanel.includes("className: 'saga-lorecard-detail-active-toggle'") &&
        lorecardsPanel.includes("actions.appendChild(createButton(entry.isPinned ? 'Unpin' : 'Pin'") &&
        lorecardsPanel.includes("actions.appendChild(createButton(entry.isSuppressed ? 'Unmute' : 'Mute'") &&
        ...
        style.includes('.saga-lorecard-workspace-row.saga-lore-entry-pinned'),
    'Active Lorecards must use row glow/filterable state without redundant active chips or left-rail bars; pinned and muted remain visible row states with detailed controls.'
);
...
assert(!lorecardsPanel.includes('This Accepted Lorecard is active.') && lorecardsPanel.includes("rows.push(['Why active', getActiveLorecardReason(entry)])") && lorecardsPanel.includes('open the Accepted Lorecards workbench.') && !lorecardsPanel.includes('accepted Lore that can affect') && !lorecardsPanel.includes('Accepted Lore workbench'), 'Active state details must use Why active and visual state instead of redundant active chips while keeping Accepted Lorecards as the visible product label.');
...
assert(liveSmoke.includes('providerCalls') && liveSmoke.includes('Lore Automation matrix completed without recording any provider calls.'), 'Live Lore Automation smoke must instrument and require real provider-call evidence.');

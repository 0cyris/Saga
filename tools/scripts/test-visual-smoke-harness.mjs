...
assert(
    mobileTouchRedesignDoc.includes('Lore | Generate | Automation')
...
assert(
    visualSmokeDoc.includes("SAGA_SMOKE_TARGET='mobile-redesign-harness'")
        ...
        && visualSmokeDoc.includes('animated secondary `Lore | Generate | Automation` sub-tab bar')
...
assert(visualSmokeDoc.includes('Lorecards secondary `Lore | Generate | Automation` sub-tab bar') && visualSmokeDoc.includes('Lorecards secondary sub-tab animation and placement'), 'Visual smoke runbook must document rendered follow-up for Lorecards secondary sub-tab navigation.');
...
assert(/\| `360x740` \| Basic \|[^\n]*Lorecards secondary `Lore \| Generate \| Automation` sub-tab bar/.test(visualSmokeDoc), 'Visual smoke runbook must document the Lorecards secondary sub-tab bar in the 360px Basic matrix.');
...
assert(lorecardsPanel.includes("const MOBILE_LORECARD_LIFECYCLE_STAGES = Object.freeze(['lore', 'generate', 'automation']);"), 'Mobile Lorecards lifecycle navigation must expose Lore, Generate, and Automation.');
for (const label of ['Lore', 'Generate', 'Automation', 'Needs Review', 'Active']) {
    assert(lorecardsPanel.includes(label), `Lorecards lifecycle label ${label} must remain visible.`);
}
assert(runtimeShell.includes("export const MOBILE_LORECARDS_STAGES = Object.freeze(['lore', 'generate', 'automation']);") && runtimeShell.includes('export function normalizeRuntimeMobileLorecardsStage') && runtimeShell.includes("if (stage === 'pending' || stage === 'accepted' || stage === 'approved' || stage === 'active' || stage === 'cards') return 'lore';") && runtimeShell.includes("if (stage === 'suggested' || stage === 'generation') return 'generate';") && runtimeShell.includes('lorecardsStage: normalizeRuntimeMobileLorecardsStage') && runtimeShell.includes('export function selectRuntimeMobileLorecardsStage(stage)'), 'Runtime shell must own the mobile Lorecards Lore, Generate, and Automation sub-tab state.');
...
assert(lorecardsPanel.includes("return (Number(stats.allPendingCount || 0) || Number(stats.acceptedCount || 0)) ? 'lore' : 'generate';") && lorecardsPanel.includes("if (lifecycleStage === 'generate')") && lorecardsPanel.includes('fragment.appendChild(createLorecardWorkspace(state, { basic, lifecycleStats }));') && lorecardsPanel.includes('scheduleMobileLorecardsStageRender(mount, state, { basic, lifecycleStats, lifecycleStage });') && lorecardsPanel.includes('function createMobileLorecardsStageContent'), 'Mobile Lorecards Lore page must own the full pending and accepted queue while mobile stage bodies render after an immediate loading shell.');
...
assert(
    defaultSettings.includes('mobileLorecardListTagsVisible: false')
        ...
        && runtimeSettingsTab.includes("toggle.className = 'saga-settings-switch-row saga-settings-mobile-tags-toggle'")
    ...
    'Settings must expose a section-heading Quality of Life area with a Theme Pack styled switch and no redundant providers/theme separator.'
);
...
assert(style.includes('container-type: inline-size') && /@container \(min-width:\s*920px\)[\s\S]*?\.saga-lorecard-workspace-body\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\)\s+minmax\(300px,\s*0\.56fr\)/.test(style) && style.includes('.saga-lorecard-detail-card .saga-lore-editor-textarea') && style.includes('.saga-lorecard-active-toggle-active'), 'Desktop Lorecards workspace must use container-aware layout, readable detail editor sizing, and a glowing active toggle.');
...
assert(
    lorecardsPanel.includes('isActive: isActiveLorecardEntry(entry)') &&
        ...
        lorecardsPanel.includes("actions.appendChild(createButton(isActiveLorecardEntry(entry) ? 'Deactivate' : 'Activate'") &&

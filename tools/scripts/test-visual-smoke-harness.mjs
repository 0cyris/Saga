...
assert(/@media \(max-width:\s*640px\)\s*\{[\s\S]*?\.saga-loredeck-creator-stage-list,[\s\S]*?\.saga-loredeck-creator-queue-row\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\);/.test(style), 'Mobile Creator workbench stages and side rows must stack to one column.');

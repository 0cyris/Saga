...
assert(lorecardsPanel.includes("if (lifecycleStage === 'automation')") && lorecardsPanel.includes('createLoreAutomationPage(state)') && lorecardsPanel.includes('createLoreAutomationCadenceSwitch') && lorecardsPanel.includes('refreshAutoRelevanceCardInPlace') && !lorecardsPanel.includes("settings.loreAutomationPaused === true ? 'Resume' : 'Pause'") && lorecardsPanel.includes("createButton('Run Now'") && lorecardsPanel.includes("createButton('Undo Last Run'") && lorecardsPanel.includes('createLoreAutomationActivityList(state)') && lorecardsPanel.includes('createLoreAutomationStateBadge(entry)'), 'Lorecards Automation tab must render the Lore Automation cockpit with Manual/Auto cadence, Run Now, Undo, and no Pause button.');
assert(lorecardsPanel.includes("const LORE_AUTOMATION_MODE_BUTTON_VALUES = Object.freeze(['ar', 'armp', 'armpc']);") && lorecardsPanel.includes('function createLoreAutomationChoiceGroup') && lorecardsPanel.includes('group.dataset.choiceGroup') && lorecardsPanel.includes("blankWhen: value => !value") && lorecardsPanel.includes("const mode = meta.active ? 'off' : normalizeLoreAutomationMode(value);"), 'Lore Automation must replace the mobile Mode dropdown with three touch buttons where no selected button means Off.');
...
assert(liveSmoke.includes('remapPreview') && liveSmoke.includes('local remap candidates') && liveSmoke.includes('applied no pin/mute changes'), 'Live Lore Automation reports must expose ARMP remap candidate diagnostics and quality notes.');

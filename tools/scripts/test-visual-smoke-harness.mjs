...
assert(style.includes('.saga-story-opener-section') && style.includes('repeat(auto-fit, minmax(min(100%, 144px), 1fr))') && style.includes('.saga-story-opener-stage-list .saga-loredeck-creator-stage-label') && style.includes('overflow-wrap: anywhere'), 'Story Opener Creator must wrap headers, workflow buttons, and stage labels inside the narrow Saga drawer.');
assert(style.includes('.saga-runtime-mobile .saga-story-opener-input-grid') && style.includes('grid-template-columns: minmax(0, 1fr)') && style.includes('.saga-runtime-mobile .saga-story-opener-quick-row') && style.includes('display: none'), 'Story Opener mobile inputs must be one column and hide the quick-shape button cluster.');

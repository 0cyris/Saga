...
assert(lorecardsPanel.includes("footer.className = 'saga-mobile-lorecard-editor-footer'") && lorecardsPanel.includes("footerActions.className = 'saga-mobile-lorecard-editor-action-slot'") && lorecardsPanel.includes('actionsContainer: footerActions') && lorecardsPanel.includes("overlay.dataset.sagaEditorReady = 'true'") && style.includes('.saga-mobile-lorecard-editor-footer') && style.includes('.saga-mobile-lorecard-editor-loading'), 'Mobile Lorecard editor persistent actions must live in a bottom footer and hydrate after the shell first paints.');
assert(liveSmoke.includes('measureMobileLorecardEditorLatency') && liveSmoke.includes('postHoldReadyMs') && liveSmoke.includes('duplicateContextmenuReopened') && liveSmoke.includes('closeGoneMs'), 'Mobile Lorecard editor smoke must measure long-press open and Close latency in the browser.');
...
assert(lorecardsPanel.includes('function createMobileLorecardStatusControls') && lorecardsPanel.includes('function attachMobileLorecardGestures') && lorecardsPanel.includes('toggleLorecardElevationByGesture(entryId, element)') && lorecardsPanel.includes('adjustLorecardRelevanceByGesture(entryId, deltaX > 0 ? 1 : -1, element)') && lorecardsPanel.includes('longPressTimer = setTimeout') && lorecardsPanel.includes('now - lastTapAt <= 320') && style.includes('.saga-mobile-lorecard-status-controls') && style.includes('.saga-mobile-lorecard-gesture-feedback') && /saga-runtime-tab-body[\s\S]*?-webkit-overflow-scrolling:\s*touch !important;[\s\S]*?touch-action:\s*pan-y !important;/.test(style), 'Mobile Lorecards must expose status controls plus double-tap Elevate, long-press Edit, and deliberate swipe relevance gestures while preserving page scrolling.');
...
assert(
    ...
    'Lorecards must use row glow/filterable Elevated state without redundant active chips or left-rail bars; relevance, mute, and Elevate remain visible row controls.'
);
...
assert(
    lorecardsPanel.includes("if (mobileShell && !entry.isPending) card.classList.add('saga-lore-entry-card-tappable');") &&
        ...
        lorecardsPanel.includes('function adjustLorecardRelevanceByGesture') &&
    ...
    'Mobile Accepted Lorecards must use visible Elevated state, compact status controls, double-tap Elevate, deliberate swipe relevance, and animated long-press editing without permanent edit/detail buttons.'
);

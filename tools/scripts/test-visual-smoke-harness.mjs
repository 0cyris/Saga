...
assert(runtimeSafetyPanel.includes('Migrate Legacy Storage') && runtimeSafetyPanel.includes('runSagaStorageMigration') && runtimeSafetyPanel.includes('getSagaStorageMigrationPlan'), 'State Safety must expose a state-manager-backed storage migration action.');
...
assert(healthPanel.includes('function createLoredeckHealthIssueActionRow') && healthPanel.includes('panel.appendChild(createLoredeckHealthIssueActionRow(group, context, issueState, editable));') && healthPanel.includes("Queue Tag ID Repair") && healthPanel.includes("Draft Repair Batches"), 'Pack Health grouped issue repair actions must be isolated behind the shared action-row helper.');
...
assert(panel.includes('LOREDECK_ASSISTANT_HEALTH_REPAIR_ISSUE_BATCH_LIMIT') && panel.includes('buildLoredeckAssistantHealthRepairBatches(selectedIssues)') && panel.includes('repairBatchCount: batches.length'), 'Pack Health assistant repair planning must batch large issue selections before model calls.');
assert(runtimeLoredeckEditorActions.includes('hydrateExternalLorepackPayloadRecord(source)') && runtimeLoredeckEditorActions.includes('validateLoredeckForEditor(fresh, null, { quiet: true, updateLibrary: false })'), 'Safe Pack Health repair must hydrate external payload-backed Loredecks before building repair writes.');
assert(panel.includes("from '../storage/saga-lorepack-payload-storage.js'") && panel.includes('async function handleLoredeckAssistantHealthRepairDraft') && panel.includes('async function queueLoredeckMalformedTagRepairFromHealthGroup') && panel.includes('hydrateExternalLorepackPayloadRecord(source)'), 'Runtime Pack Health assistant and tag repair planning must hydrate external payload-backed Loredecks before drafting or queueing repairs.');
...
assert(liveSmoke.includes('button[aria-label="Delete"]') && liveSmoke.includes('saga-confirm-overlay') && liveSmoke.includes("=== 'Confirm'"), 'Storage harness must delete the imported external Loredeck through the visible Library delete UI and Saga confirmation dialog.');
...
assert(harness.includes('__sagaSmokeStoragePersistence') && harness.includes('__sagaSmokeSettings') && harness.includes('saveSettingsDebounced()') && harness.includes('persistedSettings.themePackId'), 'Repo-local visual smoke harness must persist compact settings only for storage smoke reload checks.');

...
assert(runtimeShell.includes("export const MOBILE_LORECARDS_STAGES = Object.freeze(['generate', 'automation', 'lore']);") && runtimeShell.includes('export function normalizeRuntimeMobileLorecardsStage') && runtimeShell.includes("if (stage === 'pending' || stage === 'accepted' || stage === 'approved' || stage === 'active' || stage === 'cards') return 'lore';") && runtimeShell.includes("if (stage === 'suggested' || stage === 'generation') return 'generate';") && runtimeShell.includes('lorecardsStage: normalizeRuntimeMobileLorecardsStage') && runtimeShell.includes('export function selectRuntimeMobileLorecardsStage(stage)'), 'Runtime shell must own the mobile Lorecards Generate, Automate, and Lore sub-tab state.');
...
assert(lorecardsPanel.includes("return 'generate';") && lorecardsPanel.includes("if (lifecycleStage === 'generate')") && lorecardsPanel.includes("lifecycleStage === 'generate' ? 'Generate' : 'Capture / Suggest'") && lorecardsPanel.includes("card.appendChild(createLoreAutomationHeader(mobileShell ? 'Automate' : 'Lore Automation', settings));") && lorecardsPanel.includes('fragment.appendChild(createLorecardWorkspace(state, { basic, lifecycleStats }));') && lorecardsPanel.includes('scheduleMobileLorecardsStageRender(mount, state, { basic, lifecycleStats, lifecycleStage });') && lorecardsPanel.includes('function createMobileLorecardsStageContent'), 'Mobile Lorecards Generate is the first/default page, headings match the sub-tabs, and Lore owns the full pending/accepted queue after an immediate loading shell.');
assert(lorecardsPanel.includes("if (lifecycleStage === 'automation')") && lorecardsPanel.includes('createMobileLoreAutomationPage(state)') && lorecardsPanel.includes('createLoreAutomationCadenceSwitch') && lorecardsPanel.includes('refreshAutoRelevanceCardInPlace') && !lorecardsPanel.includes("settings.loreAutomationPaused === true ? 'Resume' : 'Pause'") && lorecardsPanel.includes("createButton('Run Now'") && lorecardsPanel.includes("createButton('Undo Last Run'") && lorecardsPanel.includes('createLoreAutomationActivityList(state)') && lorecardsPanel.includes('createLoreAutomationStateBadge(entry)'), 'Mobile Lorecards Automation tab must render the Lore Automation cockpit with Manual/Auto cadence, Run Now, Undo, and no Pause button.');
...
assert(lorecardsPanel.includes('function createLorecardWorkspace') && lorecardsPanel.includes("workspace.className = 'saga-runtime-card saga-lorecard-workspace'") && lorecardsPanel.includes("container.appendChild(createLorecardWorkspace(state, { basic, lifecycleStats }));") && !lorecardsPanel.includes('function createLorecardPipelineSurface') && !lorecardsPanel.includes('container.appendChild(createLorecardPipelineSurface'), 'Lorecards desktop route must render the unified workspace without retaining the old lifecycle pipeline surface.');

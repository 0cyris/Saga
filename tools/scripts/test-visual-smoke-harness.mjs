...
assert(
    mobileTouchRedesignDoc.includes('Generation | Automation | Pending | Approved')
...
assert(
    visualSmokeDoc.includes("SAGA_SMOKE_TARGET='mobile-redesign-harness'")
        ...
        && visualSmokeDoc.includes('mobile-redesign-360x740-06-lorecards-approved.png')
        && visualSmokeDoc.includes('selected-only reorder sheets')
        && visualSmokeDoc.includes('animated secondary `Generation | Automation | Pending | Approved` sub-tab bar')
...
assert(visualSmokeDoc.includes('Lorecards secondary `Generation | Automation | Pending | Approved` sub-tab bar') && visualSmokeDoc.includes('Lorecards secondary sub-tab animation and placement'), 'Visual smoke runbook must document rendered follow-up for Lorecards secondary sub-tab navigation.');
...
assert(/\| `360x740` \| Basic \|[^\n]*Lorecards secondary `Generation \| Automation \| Pending \| Approved` sub-tab bar/.test(visualSmokeDoc), 'Visual smoke runbook must document the Lorecards secondary sub-tab bar in the 360px Basic matrix.');
...
assert(lorecardsPanel.includes("const MOBILE_LORECARD_LIFECYCLE_STAGES = Object.freeze(['suggested', 'automation', 'pending', 'accepted']);"), 'Mobile Lorecards lifecycle navigation must expose Generation, Automation, Pending, and Approved.');
for (const label of ['Generation', 'Automation', 'Pending', 'Approved', 'Active Set']) {
    assert(lorecardsPanel.includes(label), `Lorecards lifecycle label ${label} must remain visible.`);
}
assert(runtimeShell.includes("export const MOBILE_LORECARDS_STAGES = Object.freeze(['suggested', 'automation', 'pending', 'accepted']);") && runtimeShell.includes('export function normalizeRuntimeMobileLorecardsStage') && runtimeShell.includes("if (stage === 'active') return 'accepted';") && runtimeShell.includes('lorecardsStage: normalizeRuntimeMobileLorecardsStage') && runtimeShell.includes('export function selectRuntimeMobileLorecardsStage(stage)'), 'Runtime shell must own the mobile Lorecards Generation, Automation, Pending, and Approved sub-tab state.');
...
assert(/\.saga-mobile-lorecards-subtabs\s*\{[\s\S]*?grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)[\s\S]*?animation:\s*saga-mobile-lorecards-subtabs-in/.test(style) && style.includes('.saga-runtime-mobile.saga-mobile-lorecards-subtabs-active') && style.includes('--saga-mobile-lorecards-subtab-height'), 'Mobile Lorecards sub-tabs must be a four-tab animated strip with reserved content clearance.');
assert(lorecardsPanel.includes("if (isRuntimeMobileShell() && stats.allPendingCount) return 'pending';") && lorecardsPanel.includes("if (stage === 'pending') return isRuntimeMobileShell() ? Number(stats.allPendingCount || 0) : Number(stats.pendingCount || 0);") && lorecardsPanel.includes('scheduleMobileLorecardsStageRender(mount, state, { basic, lifecycleStats, lifecycleStage });') && lorecardsPanel.includes('function createMobileLorecardsStageContent'), 'Mobile Lorecards Pending page must own the full review queue while mobile stage bodies render after an immediate loading shell.');
...
assert(lorecardsPanel.includes('function shouldShowLorecardListTags') && lorecardsPanel.includes("card.appendChild(createTagsRow(entry, { editable: !mobileShell }));") && lorecardsPanel.includes("tags.classList.add('saga-pending-readonly-tags')") && lorecardsPanel.includes('shouldShowLorecardListTags()'), 'Mobile Pending and Approved list tags must be hidden by default and read-only when enabled.');
assert(lorecardsPanel.includes("if (mobileEditor) editor.classList.add('saga-mobile-lorecard-entry-editor');") && lorecardsPanel.includes('const autosizeTextarea = (input) => {') && lorecardsPanel.includes("shell.addEventListener('pointerdown', event => event.stopPropagation())") && style.includes('.saga-mobile-lorecard-entry-editor .saga-lore-editor-textarea'), 'Mobile Lorecard editor must use larger auto-growing fields and prevent native select interactions from closing the overlay.');
...
assert(/\.saga-lore-panel\.saga-runtime-mobile \.saga-lorecards-lifecycle-tab \.saga-lore-accepted-collapsible\[open\]\s*\{[\s\S]*?overflow:\s*visible !important;/.test(style), 'Mobile Approved Lorecards section must allow long accepted lists to resize instead of clipping past the section box.');
...
assert(lorecardsPanel.includes("card.className = 'saga-runtime-card saga-lorecard-pipeline-card saga-lorecard-pipeline'") && lorecardsPanel.includes('if (!mobileShell) {\n        container.appendChild(createLorecardPipelineSurface'), 'Lorecards lifecycle pipeline must remain desktop-only and not render as the mobile stage switcher.');
assert(!lorecardsPanel.includes('saga-lorecard-pipeline-compact') && !style.includes('.saga-lorecard-pipeline-compact') && !/\.saga-runtime-mobile \.saga-lorecard-pipeline-rail/.test(style), 'Mobile Lorecards must not retain the compact lifecycle rail or mobile pipeline CSS.');
assert(!lorecardsPanel.includes("if (!stats.acceptedCount) return 'suggested';") && lorecardsPanel.includes('card.appendChild(createLorecardActiveSetSection(state, stats));') && lorecardsPanel.includes('No active Lorecards yet. Capture or suggest a Lorecard to start the review flow.') && lorecardsPanel.includes('if (!isRuntimeMobileShell()) {\n        const actions = document.createElement'), 'Blank/Approved Lorecards must show object state while mobile avoids redundant footer navigation buttons.');
...
assert(style.includes('Mobile UX Revision 3: Lorecards sub-tabs are pages, not nested list panes.') && /\.saga-lore-panel\.saga-runtime-mobile \.saga-lorecards-lifecycle-tab \.saga-pending-lore-list,[\s\S]*?\.saga-lore-panel\.saga-runtime-mobile \.saga-lorecards-lifecycle-tab \.saga-accepted-lore-section \.saga-accepted-lore-scroll-region\s*\{[\s\S]*?max-height:\s*none !important;[\s\S]*?overflow-y:\s*visible !important;[\s\S]*?scrollbar-gutter:\s*auto !important;[\s\S]*?touch-action:\s*pan-y !important;/.test(style), 'Revision 3 mobile Lorecards lists must override later desktop scroll patches and stay in the primary page scroll with finger pan support.');
...
assert(runtimeAdvancedPanel.includes('function createBasicSessionNextActionButton(readiness)') && runtimeAdvancedPanel.includes("createButton(`Next: ${row.actionLabel}`") && runtimeAdvancedPanel.includes('getBasicReadinessAction(row)') && runtimeAdvancedPanel.includes('basic && mobileRoot') && runtimeAdvancedPanel.includes("enabled && !nextAction ? 'saga-primary-button' : ''"), 'Basic mobile Session summary must expose the concrete next Session Readiness action on the root surface.');
...
assert(loredecksTabPanel.includes('let primaryMobileActionId') && loredecksTabPanel.includes("primaryMobileActionId = 'library';") && loredecksTabPanel.includes("if (primaryMobileActionId !== 'library')"), 'Mobile Loredecks root must not duplicate Open Loredeck Library when the dominant action already opens it.');
...
assert(
    lorecardsPanel.includes('Available Accepted Lorecards') &&
        ...
        lorecardsPanel.includes('attachMobileLorecardLongPress(item, () => openAcceptedLorecardMobileEditor(id))') &&
        ...
        lorecardsPanel.includes('function openAcceptedLorecardMobileEditor(entryId = \'\')') &&
        lorecardsPanel.includes("shell.className = 'saga-lore-workbench-shell saga-mobile-lorecard-editor-shell saga-mobile-touch-surface'") &&
        ...
        /function createLorecardActiveSetItem\(entry = \{\}\)[\s\S]*?if \(!isRuntimeMobileShell\(\)\) \{[\s\S]*?createButton\('Inspect'[\s\S]*?inspectAcceptedLoreEntry\(entry\.id\)[\s\S]*?createButton\(entry\.isPinned \? 'Unpin' : 'Pin'[\s\S]*?togglePinEntry\(entry\.id, \{ deferSave: true \}\);[\s\S]*?refreshLorecardLifecycleSurface\(\);[\s\S]*?createButton\('Mute'[\s\S]*?toggleSuppressEntry\(entry\.id, \{ deferSave: true \}\);[\s\S]*?refreshLorecardLifecycleSurface\(\);/.test(lorecardsPanel) &&
        /function createLorecardAvailableSetItem\(entry = \{\}\)[\s\S]*?if \(!isRuntimeMobileShell\(\)\) \{[\s\S]*?createButton\('Inspect'[\s\S]*?inspectAcceptedLoreEntry\(entry\.id\)[\s\S]*?createButton\('Activate'[\s\S]*?activateAcceptedLoreEntry\(entry\.id\)[\s\S]*?createButton\(entry\.isPinned \? 'Unpin' : 'Pin'[\s\S]*?togglePinEntry\(entry\.id, \{ deferSave: true \}\);[\s\S]*?refreshLorecardLifecycleSurface\(\);[\s\S]*?createButton\(entry\.isSuppressed \? 'Unmute' : 'Mute'[\s\S]*?toggleSuppressEntry\(entry\.id, \{ deferSave: true \}\);[\s\S]*?refreshLorecardLifecycleSurface\(\);/.test(lorecardsPanel) &&
    ...
    'Active Set must expose long-press object cards, full-window mobile editing, and visible state chips while keeping direct inspect/activation/mute/pin controls desktop-only.'
);
...
assert(
    ...
    'Mobile Approved Lorecards must use tap active-toggle and tap-hold/details inspection without a permanent card details button.'
);

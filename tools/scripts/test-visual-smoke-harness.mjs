...
assert(/\.saga-mobile-lorecards-subtabs\s*\{[\s\S]*?grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)[\s\S]*?animation:\s*saga-mobile-lorecards-subtabs-in/.test(style) && style.includes('.saga-runtime-mobile.saga-mobile-lorecards-subtabs-active') && style.includes('--saga-mobile-lorecards-subtab-height'), 'Mobile Lorecards sub-tabs must be a three-tab animated strip with reserved content clearance.');

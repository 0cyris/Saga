...
assert(style.includes('left: calc(var(--saga-rail-width, 52px) + 4px)') && style.includes('var(--saga-button,') && style.includes('var(--saga-button-hover,') && style.includes('var(--saga-surface-2,') && style.includes('var(--saga-border-soft,') && style.includes('var(--saga-border-strong,') && style.includes('var(--saga-gold-surface,'), 'Desktop Lorecards flyout must stay close to the rail and inherit the active Theme Pack through Saga shelf tokens.');
...
assert(runtimeStoryOpenerPanel.includes('STORY_OPENER_BUSY_WORDS') && runtimeStoryOpenerPanel.includes("'Making'") && runtimeStoryOpenerPanel.includes("'Crafting'") && runtimeStoryOpenerPanel.includes("'Writing'") && runtimeStoryOpenerPanel.includes("'Polishing'") && runtimeStoryOpenerPanel.includes("'Revising'") && runtimeStoryOpenerPanel.includes("'Thinking'"), 'Story Maker active provider calls must rotate a compact wait-word label.');
...
assert(runtimeStoryOpenerPanel.includes('Opening Shape Preset') && runtimeStoryOpenerPanel.includes('createSelectInput') && runtimeStoryOpenerPanel.includes('saga-story-opener-shape-select-field'), 'Story Maker must expose a mobile-friendly Opening Shape preset select.');
...
assert(style.includes('.saga-story-opener-generation-status') && style.includes('.saga-generation-live-label') && style.includes('max-width: 7rem'), 'Story Maker live generation status must use compact shared Saga live-job geometry.');
...
assert(style.includes('.saga-runtime-drawer .saga-story-opener-input-grid') && style.includes('.saga-runtime-mobile .saga-story-opener-input-grid') && style.includes('grid-template-columns: minmax(0, 1fr)') && style.includes('.saga-runtime-mobile .saga-story-opener-quick-row') && style.includes('display: none'), 'Story Maker drawer and mobile inputs must be one column and mobile must hide the quick-shape button cluster.');

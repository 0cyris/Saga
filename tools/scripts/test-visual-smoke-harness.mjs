...
assert(runtimeShellView.includes('function createMobileRouteIcon') && runtimeShellView.includes("const iconRoute = route === 'more' ? 'more' : route;") && runtimeShellView.includes('getTabIconSrc(iconRoute, settings)') && runtimeShellView.includes("iconImg.alt = ''") && runtimeShellView.includes("iconImg.setAttribute('draggable', 'false')"), 'Mobile shell routes must use the configured Saga icon assets with decorative image alt text, including the mobile More alias.');
...
assert(runtimeShellView.includes("tab.addEventListener('contextmenu', event => event.preventDefault())") && runtimeShellView.includes("entry.addEventListener('contextmenu', event => event.preventDefault())") && /saga-mobile-bottom-tab[\s\S]*?-webkit-touch-callout:\s*none;/.test(style) && /saga-mobile-bottom-icon-img[\s\S]*?-webkit-user-drag:\s*none;/.test(style), 'Mobile nav and More icon surfaces must suppress Android image callouts and drag/download prompts.');
...
assert(runtimeShellView.includes("if (mobile.activeRoute === 'more' && !mobile.activeMoreRoute) return null;"), 'The More index must not render a redundant shell Back button.');
assert(runtimeShellView.includes('function renderMobileMoreSheet') && runtimeShellView.includes("sheet.setAttribute('role', 'menu')") && runtimeShellView.includes("sheet.setAttribute('aria-label', 'More Saga routes')") && runtimeShellView.includes("modeLabel.textContent = 'Experience Mode'") && runtimeShellView.includes("heading.textContent = group.label") && runtimeShellView.includes("entry.setAttribute('role', 'menuitem')") && runtimeShellView.includes("entry.dataset.mobileMoreRoute = route") && runtimeShellView.includes('getMobileRouteLabel(route, settings)') && runtimeShellView.includes('selectRuntimeMobileMoreRoute(route)') && runtimeShellView.includes("exitText.textContent = 'Exit Saga'") && runtimeShellView.includes('hideRuntimeMobileShell();'), 'Mobile More sheet must remain a grouped routed menu surface, label Experience Mode, and own a clear Exit Saga shell action.');
...
assert(style.includes('.saga-mobile-bottom-exit-icon') && style.includes('.saga-mobile-more-mode-label') && /\.saga-mobile-more-mode\s*\{[\s\S]*?align-self:\s*center;[\s\S]*?justify-items:\s*center;/.test(style), 'Mobile bottom Exit icon and centered More Experience Mode label styles must be present.');
...
assert(themeActions.includes('refreshPanelBody({ preserveScroll: true, preserveWindowScroll: true });') && runtimeTheme.includes("'tab.more': 'tab.settings'") && runtimeTheme.includes("more: 'tab.settings'"), 'Changing Icon Set must refresh mobile navigation surfaces and resolve the mobile More icon through icon-set aliases.');
...
assert(
    lorecardsPanel.includes('Available Accepted Lorecards') &&
        ...
        lorecardsPanel.includes("selectRuntimeMobileLorecardsStage('accepted')") &&
    ...
    'Active Set must expose tappable object cards and visible state chips on mobile while keeping direct inspect/activation/mute/pin controls desktop-only.'
);

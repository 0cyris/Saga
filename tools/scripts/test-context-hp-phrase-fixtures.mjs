...
import { resolveContextsFromContext } from '../../src/context/context-resolver.js';
...
async function buildRegistryRecord(packId) {
    const manifest = await readJson(`content/loredecks/${packId}/loredeck.json`);
    const timelineRegistry = await readJson(`content/loredecks/${packId}/timeline.json`);
    return {
        packId,
        id: packId,

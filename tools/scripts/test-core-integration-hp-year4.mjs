...
const YEAR_4_PACK_ID = 'hp-year-4-goblet-of-fire';
const LOADED_PACKS = [CORE_PACK_ID, YEAR_4_PACK_ID];
const EARLY_ENTRY_ID = 'gof_triwizard_selection_constraints';
const LATE_ENTRY_ID = 'gof_graveyard_voldemort_return_state';
const LATE_LEXICON_ENTRY_ID = 'lexcal_y4_third_task_graveyard_return';
...
function buildState(getDefaultState) {
  ...
  state.loredeckStack = [
    ...
    { packId: YEAR_4_PACK_ID, enabled: true, priority: 100, addedAt: 2 },
  ];
  ...
  state.scene = {
    ...state.scene,
    presentCharacters: ['Harry Potter', 'Cedric Diggory', 'Albus Dumbledore'],
    nearbyCharacters: ['Ron Weasley', 'Hermione Granger', 'Alastor Moody'],
    location: 'Hogwarts',
    currentActivity: 'Year 4 Triwizard Tournament integration test',
  };
  return state;
}
...
function applyResolvedContexts(state, resolution, sourceContext) {
  ...
  assert.ok(resolved.some(result => result.packId === YEAR_4_PACK_ID), 'Expected Year 4 Context to resolve.');
  ...
  const year4 = state.loredeckContexts[YEAR_4_PACK_ID];
  state.loredeckContexts[CORE_PACK_ID] = {
    ...(state.loredeckContexts[CORE_PACK_ID] || {}),
    packId: CORE_PACK_ID,
    contextType: 'calendar',
    label: `Core mirror for ${year4.label || YEAR_4_PACK_ID}`,
    sceneDate: sourceContext.sceneDate,
    canonBoundary: sourceContext.canonBoundary,
    contextSortKey: year4.contextSortKey,
    contextSortKeyFrom: year4.contextSortKeyFrom ?? year4.contextSortKey,
    contextSortKeyTo: year4.contextSortKeyTo ?? year4.contextSortKey,
...
function assertOnlyLoadedPacks(entries = []) {
  ...
  assert.deepEqual(unexpected, [], 'Preview should only include entries from the explicitly loaded Year 4 stack.');
}
...
function assertMemoHasCurrentYear4Aftermath(memo = '') {
  assert.ok(
    cleanText(memo).includes('Cedric is dead, Voldemort has returned physically'),
    `Lore memo should include the current Year 4 aftermath substance: ${memo}`,
  );
}
...
async function main() {
  ...
  assert.equal(contextIndex.summary.packCount, 2, 'Expected hp-core and hp-year-4 context indexes.');
  assert.ok(contextIndex.summary.anchorCount >= 56, 'Expected a dense Year 4 Context anchor index.');
  assert.ok(contextIndex.summary.windowCount >= 9, 'Expected a meaningful Year 4 Context window index.');

  const earlyContext = {
    sceneDate: 'Friday, Oct 28, 1994',
    canonBoundary: 'Goblet of Fire Year 4, Triwizard Tournament announced, Age Line and champion selection pressure before later tasks',
    branchId: 'main',
  };
  ...
  const earlyYear4 = early.resolution.results.find(result => result.packId === YEAR_4_PACK_ID);
  assert.equal(earlyYear4?.anchor?.id, 'hp.y4.moody_unforgivables_lesson');
  const earlyIds = entryIds(early.preview.entries);
  assert.equal(earlyIds.has(EARLY_ENTRY_ID), true, 'Early Year 4 preview should include Triwizard selection constraints.');
  assert.equal(earlyIds.has(LATE_ENTRY_ID), false, 'Early Year 4 preview should block graveyard aftermath lore.');
  assert.equal(earlyIds.has(LATE_LEXICON_ENTRY_ID), false, 'Early Year 4 preview should block third-task graveyard reveal lore.');

  const earlyEntry = pickEntry(early.preview, EARLY_ENTRY_ID);
  forceHigh(earlyEntry);
  appendPendingLoreEntries([earlyEntry], {
    source: 'integration_year4_early',
    contextKey: 'hp-year4-triwizard-selection',
    rawEntryCount: 1,
  ...
  assert.ok(earlyAccepted, 'Early Year 4 Lorecard should be accepted.');
  ...
  assert.ok(earlyMemo.includes(injectionText(earlyAccepted)), 'Accepted early Year 4 Lorecard should inject while its Context gate matches.');

  const lateContext = {
    sceneDate: 'Sunday, Jun 25, 1995',
    canonBoundary: 'Goblet of Fire Year 4 after the graveyard, Cedric death, Voldemort return, Dumbledore reactivates the Order, and the train home aftermath',
    branchId: 'main',
  };
  ...
  const lateYear4 = late.resolution.results.find(result => result.packId === YEAR_4_PACK_ID);
  assert.equal(lateYear4?.anchor?.id, 'hp.y4.train_home_after_cedric');
  const lateIds = entryIds(late.preview.entries);
  assert.equal(lateIds.has(EARLY_ENTRY_ID), false, 'Late Year 4 preview should block earlier Triwizard selection constraints.');
  assert.equal(lateIds.has(LATE_ENTRY_ID), true, 'Late Year 4 preview should include graveyard aftermath lore.');
  assert.equal(lateIds.has(LATE_LEXICON_ENTRY_ID), true, 'Late Year 4 preview should include third-task graveyard reveal lore.');

  const lateEntry = pickEntry(late.preview, LATE_ENTRY_ID);
  forceHigh(lateEntry);
  appendPendingLoreEntries([lateEntry], {
    source: 'integration_year4_late',
    contextKey: 'hp-year4-graveyard-aftermath',
    rawEntryCount: 1,
    normalizedEntryCount: 1,
  }, { syncPrompt: false, full: true });
  acceptPendingLoreEntries();
  ...
  assert.ok(staleAccepted, 'Earlier Year 4 Lorecard should remain accepted after Context advances.');
  assert.ok(currentAccepted, 'Later Year 4 Lorecard should be accepted after Context advances.');
  ...
  assert.equal(staleGate.eligible, false, `Earlier Year 4 Lorecard should be Context-blocked after Context advances: ${JSON.stringify(staleGate)}`);
  assert.equal(currentGate.eligible, true, `Current Year 4 Lorecard should remain eligible: ${JSON.stringify(currentGate)}`);

  const lateMemo = buildLoreMemo(accepted, getSettings());
  assert.ok(!lateMemo.includes(injectionText(staleAccepted)), 'Accepted stale Year 4 Lorecard should not inject outside its active Loredeck Context.');

  const audit = buildLoreInjectionAudit(accepted, getSettings(), {
    transport: 'integration_year4',
  ...
  assert.equal(staleAudit?.decision, 'context_blocked', `Expected stale Year 4 Lorecard to be audited as context_blocked: ${JSON.stringify(staleAudit)}`);
  assert.equal(currentAudit?.decision, 'injected', `Expected current Year 4 Lorecard to inject: ${JSON.stringify(currentAudit)}`);
  assertMemoHasCurrentYear4Aftermath(lateMemo);
  assert.equal(audit.summary.contextBlocked >= 1, true, 'Injection audit should count Context-blocked Year 4 Lorecards.');

  console.log(JSON.stringify({
    ok: true,
    scenario: 'hp-year4-context-progression-injection',
    contexts: {
      early: earlyYear4.anchor?.id || earlyYear4.window?.id || '',
      late: lateYear4.anchor?.id || lateYear4.window?.id || '',
    },
    entries: {

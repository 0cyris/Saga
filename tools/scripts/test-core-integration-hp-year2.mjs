...
const YEAR_2_PACK_ID = 'hp-year-2-chamber-of-secrets';
const LOADED_PACKS = [CORE_PACK_ID, YEAR_2_PACK_ID];
const EARLY_ENTRY_ID = 'cos_chamber_attack_progression';
const EARLY_GUARD_ENTRY_ID = 'cos_diary_ginny_possession_guard';
const LATE_ENTRY_ID = 'event_chamber_resolved';
const LATE_BLOCKED_ENTRY_ID = 'cos_chamber_attack_progression';
...
function buildState(getDefaultState) {
  ...
  state.loredeckStack = [
    ...
    { packId: YEAR_2_PACK_ID, enabled: true, priority: 100, addedAt: 2 },
  ];
  ...
  state.scene = {
    ...state.scene,
    presentCharacters: ['Harry Potter', 'Ron Weasley', 'Hermione Granger', 'Ginny Weasley'],
    nearbyCharacters: ['Gilderoy Lockhart', 'Dobby', 'Tom Riddle'],
    location: 'Hogwarts',
    currentActivity: 'Year 2 Chamber of Secrets integration test',
  };
  return state;
}
...
function applyResolvedContexts(state, resolution, sourceContext) {
  ...
  assert.ok(resolved.some(result => result.packId === YEAR_2_PACK_ID), 'Expected Year 2 Context to resolve.');
  ...
  const year2 = state.loredeckContexts[YEAR_2_PACK_ID];
  state.loredeckContexts[CORE_PACK_ID] = {
    ...(state.loredeckContexts[CORE_PACK_ID] || {}),
    packId: CORE_PACK_ID,
    contextType: 'calendar',
    label: `Core mirror for ${year2.label || YEAR_2_PACK_ID}`,
    sceneDate: sourceContext.sceneDate,
    canonBoundary: sourceContext.canonBoundary,
    contextSortKey: year2.contextSortKey,
    contextSortKeyFrom: year2.contextSortKeyFrom ?? year2.contextSortKey,
    contextSortKeyTo: year2.contextSortKeyTo ?? year2.contextSortKey,
...
function assertOnlyLoadedPacks(entries = []) {
  ...
  assert.deepEqual(unexpected, [], 'Preview should only include entries from the explicitly loaded Year 2 stack.');
}
...
function assertMemoHasCurrentYear2Aftermath(memo = '') {
  assert.ok(
    cleanText(memo).includes('the basilisk is defeated, the diary is destroyed, Ginny is rescued'),
    `Lore memo should include the current Year 2 Chamber aftermath substance: ${memo}`,
  );
}
...
async function main() {
  ...
  assert.equal(contextIndex.summary.packCount, 2, 'Expected hp-core and hp-year-2 context indexes.');
  assert.ok(contextIndex.summary.anchorCount >= 55, 'Expected a dense Year 2 Context anchor index.');
  assert.ok(contextIndex.summary.windowCount >= 9, 'Expected a meaningful Year 2 Context window index.');

  const earlyContext = {
    sceneDate: '1992-12-16',
    canonBoundary: 'Chamber of Secrets Year 2 after the first attack and Mrs Norris petrification, before Ginny/Riddle/basilisk reveal',
    branchId: 'main',
  };
  ...
  const earlyYear2 = early.resolution.results.find(result => result.packId === YEAR_2_PACK_ID);
  assert.equal(earlyYear2?.anchor?.id, 'hp.y2.mrs_norris_petrified');
  const earlyIds = entryIds(early.preview.entries);
  assert.equal(earlyIds.has(EARLY_ENTRY_ID), true, 'Early Year 2 preview should include active Chamber attack progression lore.');
  assert.equal(earlyIds.has(EARLY_GUARD_ENTRY_ID), true, 'Early Year 2 preview should include the Ginny/Riddle/basilisk pre-reveal guard.');
  assert.equal(earlyIds.has(LATE_ENTRY_ID), false, 'Early Year 2 preview should block Chamber resolution aftermath lore.');

  const earlyEntry = pickEntry(early.preview, EARLY_ENTRY_ID);
  forceHigh(earlyEntry);
  appendPendingLoreEntries([earlyEntry], {
    source: 'integration_year2_early',
    contextKey: 'hp-year2-chamber-attack-progression',
    rawEntryCount: 1,
  ...
  assert.ok(earlyAccepted, 'Early Year 2 Lorecard should be accepted.');
  ...
  assert.ok(earlyMemo.includes(injectionText(earlyAccepted)), 'Accepted early Year 2 Lorecard should inject while its Context gate matches.');

  const lateContext = {
    sceneDate: '1993-06-08',
    canonBoundary: 'Chamber of Secrets Year 2 after basilisk confrontation, diary destruction, Ginny rescued, and Chamber resolution',
    branchId: 'main',
  };
  ...
  const lateYear2 = late.resolution.results.find(result => result.packId === YEAR_2_PACK_ID);
  assert.equal(lateYear2?.anchor?.id, 'hp.y2.ginny_rescued');
  const lateIds = entryIds(late.preview.entries);
  assert.equal(lateIds.has(EARLY_ENTRY_ID), false, 'Late Year 2 preview should block the earlier pre-reveal guard.');
  assert.equal(lateIds.has(EARLY_GUARD_ENTRY_ID), false, 'Late Year 2 preview should block the Ginny/Riddle/basilisk pre-reveal guard.');
  assert.equal(lateIds.has(LATE_BLOCKED_ENTRY_ID), false, 'Late Year 2 preview should block broad active Chamber attack progression lore after resolution.');
  assert.equal(lateIds.has(LATE_ENTRY_ID), true, 'Late Year 2 preview should include Chamber resolution aftermath lore.');

  const lateEntry = pickEntry(late.preview, LATE_ENTRY_ID);
  forceHigh(lateEntry);
  appendPendingLoreEntries([lateEntry], {
    source: 'integration_year2_late',
    contextKey: 'hp-year2-chamber-resolution',
    rawEntryCount: 1,
    normalizedEntryCount: 1,
  }, { syncPrompt: false, full: true });
  acceptPendingLoreEntries();
  ...
  assert.ok(staleAccepted, 'Earlier Year 2 Lorecard should remain accepted after Context advances.');
  assert.ok(currentAccepted, 'Later Year 2 Lorecard should be accepted after Context advances.');
  ...
  assert.equal(staleGate.eligible, false, `Earlier Year 2 Lorecard should be Context-blocked after Context advances: ${JSON.stringify(staleGate)}`);
  assert.equal(currentGate.eligible, true, `Current Year 2 Lorecard should remain eligible: ${JSON.stringify(currentGate)}`);

  const lateMemo = buildLoreMemo(accepted, getSettings());
  assert.ok(!lateMemo.includes(injectionText(staleAccepted)), 'Accepted stale Year 2 Lorecard should not inject outside its active Loredeck Context.');

  const audit = buildLoreInjectionAudit(accepted, getSettings(), {
    transport: 'integration_year2',
  ...
  assert.equal(staleAudit?.decision, 'context_blocked', `Expected stale Year 2 Lorecard to be audited as context_blocked: ${JSON.stringify(staleAudit)}`);
  assert.equal(currentAudit?.decision, 'injected', `Expected current Year 2 Lorecard to inject: ${JSON.stringify(currentAudit)}`);
  assertMemoHasCurrentYear2Aftermath(lateMemo);
  assert.equal(audit.summary.contextBlocked >= 1, true, 'Injection audit should count Context-blocked Year 2 Lorecards.');

  console.log(JSON.stringify({
    ok: true,
    scenario: 'hp-year2-context-progression-injection',
    contexts: {
      early: earlyYear2.anchor?.id || earlyYear2.window?.id || '',
      late: lateYear2.anchor?.id || lateYear2.window?.id || '',
    },
    entries: {
      ...
      earlyGuardBlockedAfterReveal: EARLY_GUARD_ENTRY_ID,
      blockedAftermathAsserted: LATE_BLOCKED_ENTRY_ID,
    },

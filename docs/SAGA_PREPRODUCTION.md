...
36. Next: run a targeted visual smoke pass in SillyTavern against these fixes, including collapsed-section defaults, Reset Window behavior, Loredeck chip/title styling, HP Deck Health malformed namespace warnings, Creator wizard launch, granularity copy, stack arrows, runtime header branding, and the new Health Center.
37. Next: expand Deck Health remediation: deterministic Custom-deck tag ID normalization, clearer bundled-deck duplicate-to-edit flow, ignore/resolve states, and assistant repair handoff from grouped issues.
38. Next: design and test individual Loredeck bulk export/import. Export should operate from selected Loredecks, produce one JSON file per deck, avoid whole-library export/import, and treat URL/GitHub updates as a previewed import/reinstall flow that protects locally modified decks.
39. Next: complete a scroll-preservation audit for Loredecks actions that still trigger full panel refreshes or snap the runtime window to the top.
40. Next: run UX/UI studies before major implementation for the fullscreen Loredeck Library + Stack Loader workbench and the Story Position editor. Deck Health has a first Health Center implementation but still needs richer coverage rules and true repair workflows.

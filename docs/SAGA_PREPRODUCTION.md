...
45. In progress: expand Deck Health remediation from diagnosis into action. The first slice queues deterministic malformed tag ID repairs for editable Custom/Generated Loredecks as Pending Review proposals, preserves user approval, marks health-impacting accepted changes stale through the existing Pending Review path, and gives Bundled decks a direct Duplicate-as-Custom route before repair.
46. Next: finish the Deck Health remediation loop with explicit ignore/resolve states, grouped-issue assistant handoff from the Health Center, and clearer post-repair rerun guidance.
47. Next: redesign the Story Position editor before deeper implementation. Replace the cramped grid with a dense spreadsheet/timeline workbench for anchors, windows, coordinates, aliases, bulk edits, filtering, validation, and resolver testing.

...
46. Next: redesign the Story Position editor before deeper implementation. Replace the cramped grid with a dense spreadsheet/timeline workbench for anchors, windows, coordinates, aliases, bulk edits, filtering, validation, and resolver testing.
47. Next: implement durable asset/bundle handling for Custom Loredecks, Theme Packs, Icon Sets, and deck covers. Decide zip versus SillyTavern-managed storage, validate allowed image types, block executable content, preserve deck-cover paths, and make imported visual assets survive browser sessions.

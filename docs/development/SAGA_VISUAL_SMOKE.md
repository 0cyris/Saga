...
| `430x820` | Advanced | Advanced icon-only bottom nav with Loredecks, Session, Continuity, Context, Lorecards, Injection, Settings; Lorecards secondary sub-tabs, unified Lore object list, active/pinned/muted visual states, Accepted tap/tap-hold/detail affordances, Pack Health tabs and bottom actions, Creator current-task priority and bottom actions, Library object actions, Context proposal review bottom actions. |
...
Rendered Accepted object-action review should confirm unified Lore rows expose object-state chips plus tap/tap-hold/detail affordances for activate, pin, mute, and inspection without showing equal permanent row buttons across every card.
...
It verifies the mobile Library browse surface hides desktop stack/details/resize/grab handles, deck taps update active-stack order badges, the selected strip opens detail and selected-only reorder sheets, the Lorecards route shows an animated secondary `Lore | Generate | Automation` sub-tab bar above the fixed bottom nav, the Lorecards page body does not use a `Lorecard Pipeline` card or lifecycle button row as the stage switcher, Lore combines Pending Review and Accepted/Active Lorecards in one object list, Pending Review rows expose only Accept and Reject, Accepted rows use readable two-line titles and state/detail affordances instead of full row-action buttons, Generate stays focused on creation/suggestion, Automation stays focused on automation status and controls, and all checked surfaces avoid horizontal overflow.

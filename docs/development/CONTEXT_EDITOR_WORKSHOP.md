...
The Context tab now has a consolidated `Choose Story Position` surface above the manual editor. It treats first-class timeline anchors/windows as the major story-position list, supports exact timeline-row search, keeps the current row visible when possible, and can load Lorecards on demand to expose event-level Context gates as Lorecard-derived story events. The expected runtime gesture is:

1. Search or browse the Loredeck's major story positions.
2. Click `Start Here` for an exact anchor, click `Use Window` for a predefined range, or click `After` on one story position and `Before` on another to create a bounded Context window.
...
Do not expand this into an exhaustive local alias system. The long-term registry policy should add durable anchors for high-value recurring story moments, Deck Maker output, accepted suggestions, and sparse coverage warnings. Casual phrasing should be handled by the Reasoner Provider selecting from bounded known candidates.

Deck Health now supports that policy with advisory densification signals. It can suggest more durable timeline candidates when a deck has many Context-gated Lorecards but very few anchors/windows, when many gates cluster around only a few anchors, or when a timeline has anchors but no broad selectable windows. These are suggestions only; they should guide authors toward useful waypoints without blocking sparse, scenario-first, or intentionally broad Custom decks.

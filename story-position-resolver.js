/**
 * story-position-resolver.js -- Saga/Wandlight
 * Local, non-model Story Position resolver for enabled Lorepacks.
 */
...
import {
    getStoryPositionIndexSync,
    loadStoryPositionIndex,
    rankStoryPositionAnchors,
} from './story-position-index.js';
...
const STORY_POSITION_MODEL_SYSTEM_PROMPT = `You are the Saga Story Position Resolver.

Resolve each target Lorepack to one known timeline anchor using only the anchor IDs provided.
  ...
  "positions": [
    {
      ...
      "anchorId": "string",
      "label": "string",
      "positionType": "calendar|anchor|anchor_window|arc|phase|season_episode|stardate|relative|hybrid|custom",
      "sceneDate": "string",
      "alias": "string",
      "confidence": 0.0,
      "reason": "short string"
...
function buildResolutionFromMatch(packId, match, context = {}, options = {}) {
    const patch = buildStoryPositionPatchFromAnchor(match.anchor, {
...
function getBestAliasMatch(packId, context = {}, index = {}, options = {}) {
    ...
    const ranked = rankStoryPositionAnchors(resolverText, { packId, index, limit: 5 });
    const top = ranked[0];
...
function currentPositionForPack(state = {}, packId = '') {
    const contexts = isPlainObject(state?.lorepackContexts) ? state.lorepackContexts : {};
    return contexts[packId] || getLorepackStoryPosition(state, packId);
}
...
function parseStoryPositionModelResponse(text = '') {
    const candidates = [
...
function resolveAnchorForModelChoice(choice = {}, index = {}) {
    ...
    return rankStoryPositionAnchors(label, { packId, index, limit: 1 })[0]?.anchor || null;
}

function buildStoryPositionPatchFromModelChoice(choice = {}, anchor = {}, context = {}) {
    const confidence = clampConfidence(choice.confidence, 0.6);
    const patch = buildStoryPositionPatchFromAnchor(anchor, {
        contextSource: 'model',
        confidence,
    });
    patch.positionType = cleanString(choice.positionType || patch.positionType, 80) || patch.positionType;
...
function buildStoryPositionModelPrompt(context = {}, options = {}) {
        ...
        .map(item => {
            ...
            const anchors = getPackAnchors(index, item.packId)
                ...
                .map(anchor => ({
                    id: anchor.id,
                    label: anchor.label,
                    positionType: anchor.positionType,
    ...
    return JSON.stringify({
        task: 'Resolve Story Position for target Lorepacks using only listed anchors.',
        currentStoryContext: {
...
export function resolveStoryPositionsFromModelResponse(responseText = '', context = {}, options = {}) {
    const state = options.state || getState();
    const index = options.index || getStoryPositionIndexSync();
    const targetPackIds = new Set((options.targetPackIds || []).map(packId => cleanString(packId, 160)).filter(Boolean));
    const minConfidence = clampConfidence(options.minConfidence, MIN_MODEL_CONFIDENCE);
    const parsed = parseStoryPositionModelResponse(responseText);
    const results = [];
    ...
    for (const rawChoice of parsed.positions || []) {
        ...
        const patch = buildStoryPositionPatchFromModelChoice(rawChoice, anchor, context);
        results.push({
            packId,
            status: 'resolved',
            matchType: 'model',
...
export function resolveStoryPositionsFromContext(context = {}, options = {}) {
    const state = options.state || getState();
    const index = options.index || getStoryPositionIndexSync();
...
export async function resolveAndApplyStoryPositionsFromContext(context = {}, options = {}) {
    const index = options.index || getStoryPositionIndexSync() || await loadStoryPositionIndex();
    const state = options.state || getState();
    const resolution = resolveStoryPositionsFromContext(context, {
        ...options,
        state,
        index,
    });
    ...
    for (const result of resolution.results || []) {
        ...
        setLorepackStoryPosition(result.packId, {
            ...result.patch,
            updatedAt: Date.now(),
        });
        appliedCount += 1;
...
export async function resolveStoryPositionsWithModel(context = {}, options = {}) {
    const index = options.index || getStoryPositionIndexSync() || await loadStoryPositionIndex();
    const state = options.state || getState();
    const local = resolveStoryPositionsFromContext(context, {
        ...options,
        state,
        index,
    });
    ...
    const userPrompt = buildStoryPositionModelPrompt(context, {
        ...options,
        state,
        index,
        targetPackIds,
    });
    const responseText = await sendLoreRequest(STORY_POSITION_MODEL_SYSTEM_PROMPT, userPrompt, {
        providerKind: 'lore',
        expectedOutput: 'json',
        maxTokens: options.maxTokens || 1800,
        signal: options.signal || null,
    });
    const model = resolveStoryPositionsFromModelResponse(responseText, context, {
        ...options,
        state,
        index,
        targetPackIds,
...
export const __storyPositionResolverTestHooks = {
    parseDateLike,
    parseContextDate,
    buildResolverText,
    scoreAnchorDateMatch,
    resolveStoryPositionsFromContext,
    buildStoryPositionPatchFromAnchor,
    buildStoryPositionModelPrompt,
    parseStoryPositionModelResponse,
    resolveStoryPositionsFromModelResponse,
    buildStoryPositionPatchFromModelChoice,
};
